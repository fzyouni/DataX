...
public class SensorsIdentity implements Serializable {
  ...
  public Pair<String, String> convertIdentity(Map<String, Object> fieldMaps) {
    String key =
        StringUtils.isNotBlank(sensorsIdKeyName) ? sensorsIdKeyName : fieldMaps.remove(sensorIdKeyColumn).toString();
    String value =
        StringUtils.isNotBlank(sensorIdValueName) ?
            sensorIdValueName : fieldMaps.remove(sensorIdValueColumn).toString();
